...
<template>
    ...
    >
        ...
        >
            <!-- Left Slot (Multiple logos + Vertical Dividers) -->
            <div class="flex justify-start gap-4 items-center flex-shrink-0">
                ...
                <!-- Vertical Dividers block -->
                <div 
                    v-if="settings.verticalDivider?.enabled ?? true" 
                    class="flex items-center flex-shrink-0 pl-2"
                    :style="{ 
                        height: `${settings.logoSize ?? 80}px`,
                        gap: `${settings.verticalDivider?.gap ?? 6}px` 
                    }"
                >
                    <!-- Vertical Divider 1: Thin line -->
                    <div 
                        class="h-full" 
                        :style="{ 
                            width: `${settings.verticalDivider?.thinWidth ?? 2}px`,
                            backgroundColor: settings.verticalDivider?.thinColor || settings.verticalDivider?.color || settings.textColor || '#000000' 
                        }"
                    ></div>
                    <!-- Vertical Divider 2: Thick bar -->
                    <div 
                        class="h-full" 
                        :style="{ 
                            width: `${settings.verticalDivider?.thickWidth ?? 5}px`,
                            backgroundColor: settings.verticalDivider?.thickColor || settings.verticalDivider?.color || settings.textColor || '#000000' 
                        }"
                    ></div>
                </div>
            </div>
